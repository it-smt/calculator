...
class MainWindow(QWidget):
	...
	def get_result(self):
		...
		if "." in self.number or "." in self.number2:
			try:
				if self.expression == "+":
					self.result = f"{float(self.number) + float(self.number2)}"
			...
			try:
				if self.expression == "+":
					self.result = f"{int(self.number) + int(self.number2)}"
				elif self.expression == "-":
					self.result = f"{int(self.number) - int(self.number2)}"
				elif self.expression == "*":
					self.result = f"{int(self.number) * int(self.number2)}"
				elif self.expression == "/":
					self.result = f"{int(int(self.number) / int(self.number2))}"
			except ZeroDivisionError:
				self.result = "E"
	...
	def use_operation(self, operation):
		""" Функция дял присваивания спец. символа к переменной и вывода пустой строки"""
		self.expression = operation
		self.lcd.display("")

	def use_numbers(self, number):
		"""Функция для добавления числа в переменную"""
		if self.expression != "":
			self.number2 += number
		else:
			self.number += number
